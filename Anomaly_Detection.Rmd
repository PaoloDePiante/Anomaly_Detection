...
So, I elaborated .txt files to merge them in a single .csv file operating a little resampling of data. The final data frame has 6324 rows and 5 columns (TimeStamp and four Bearings Vibration). The goal of this analysis is to demostrate how the Machine Learning algorithms can detect engine feilures in advance. This study can be applied to any kind of engine using special sensors named "Accelerometer" (in our scenario four of them) oriented toward the engine to catch its vibrations.
